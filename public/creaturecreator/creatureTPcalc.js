...
import { calculatePowerCosts } from '../scripts/power_calc.js';
import { calculateTechniqueCosts } from '../scripts/technique_calc.js';
    ...
    creatureData.powersTechniques.forEach(item => {
      if (item.type === 'power') {
        ...
        (item.parts || []).forEach(partData => {
          const partDef = powerPartsDb.find(p => p.name === partData.name);
          if (!partDef) return;
  ...
  if (creatureData.powersTechniques && Array.isArray(creatureData.powersTechniques)) {
    creatureData.powersTechniques.forEach(item => {
      if (item.type === 'technique') {
        ...
        (item.parts || []).forEach(partData => {
          const partDef = techniquePartsDb.find(p => p.name === partData.name);
          if (!partDef) return;
          const l1 = partData.op_1_lvl || 0;
          const l2 = partData.op_2_lvl || 0;
          const l3 = partData.op_3_lvl || 0;
