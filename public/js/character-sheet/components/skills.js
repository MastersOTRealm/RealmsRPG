...
function getSpeciesSkills(charData, speciesData) {
    ...
    const speciesEntry = Object.values(speciesData).find(s => s.name === charData.species);
...
function isSubSkill(skillName, skillsData) {
    const skillEntry = Object.values(skillsData || {}).find(s => s.name === skillName);
...
export async function renderSkills(charData) {
    ...
    if (subSkills.length > 0) {
        subSkills.forEach((subSkill) => {
            ...
            const baseSkillObj = baseSkills.find(s => s.name === subSkill.baseSkill);
        ...
        dot.addEventListener('click', () => {
            ...
            if (charData.skills && charData.skills[idx]) {
                ...
                if (baseSkillName) {
                    ...
                    const baseSkill = allSkills.find(s => s.name === baseSkillName);
